...
class StateMetricSharedInfoProvider(MetricSharedInfoProvider[AgentID, GameState], ABC):
    ...
    def init_metric_value(self, initial_state: GameState, shared_info: Dict[str, Any]) -> float | None:
        """
    ...
    def set_state(self, agents: List[AgentID], initial_state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        shared_info[METRICS_HEADER][STATE_METRICS_HEADER][self.metric_name] = self.init_metric_value(initial_state, shared_info)
        return shared_info

    def step(self, agents: List[AgentID], state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        shared_info[METRICS_HEADER][STATE_METRICS_HEADER][self.metric_name] = self.get_metric_value(state, shared_info)
        return shared_info
...
class PlayerMetricSharedInfoProvider(MetricSharedInfoProvider[AgentID, GameState], ABC):
    ...
    def init_metric_value_for(self, agent: AgentID, initial_state: GameState, shared_info: Dict[str, Any]) -> float | None:
        """
    ...
    def set_state(self, agents: List[AgentID], initial_state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        for agent in agents:
            shared_info[METRICS_HEADER][PLAYERS_METRICS_HEADER][self.metric_name][agent] = self.init_metric_value_for(agent, initial_state, shared_info)
        return shared_info

    def step(self, agents: List[AgentID], state: GameState, shared_info: Dict[str, Any]) -> Dict[str, Any]:
        for agent in agents:
            shared_info[METRICS_HEADER][PLAYERS_METRICS_HEADER][self.metric_name][agent] = self.get_metric_value_for(agent, state, shared_info)
        return shared_info